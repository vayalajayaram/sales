...
class Staff_Model extends CI_Model {
	...
	function get_org( $id ) {
		$this->db->select( '*,plans.plan_type as plan, organizations.org_id as id' );
		$this->db->join( 'plans', 'organizations.plan_id = plans.plan_id', 'left' );
		return $this->db->get_where( 'organizations', array( 'organizations.org_id' => $id ) )->row_array();
	}
	...
	function get_all_organization() {
		$this->db->select( '*,plans.plan_type as plan, organizations.org_id as id' );
		$this->db->join( 'plans', 'organizations.plan_id = plans.plan_id', 'left' );
		return $this->db->get_where( 'organizations', array( '' ) )->result_array();
	}
	...
	function total_leads_by_staff( $id ) {
		$this->db->from( 'leads' );
	...
	function isDuplicateorg( $email ) {
		$this->db->get_where( 'organizations', array( 'email' => $email ), 1 );
		return $this->db->affected_rows() > 0 ? TRUE : FALSE;
	}
