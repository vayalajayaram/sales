...
class Leads_Model extends CI_Model {
	...
	function get_lead( $id ) {
		...
		$this->db->join( 'remarks', 'leads.id = remarks.lead_id', 'left' );
		return $this->db->get_where( 'leads', array( 'leads.id' => $id ) )->row_array();
	}
	function get_remarks( $id ) {
		return $this->db->get_where( 'remarks', array( 'lead_id' => $id ) )->row_array();
	}
	...
	function get_all_leads() {
		...
		$this->db->join( 'leadssources', 'leads.source = leadssources.id', 'left' );
		$this->db->join( 'staff', 'leads.assigned_id = staff.id', 'left' );
		$this->db->where( 'change = 0  AND assigned_id = '.$this->session->userdata( 'usr_id' ) .'' );
		//$this->db->where( 'status = 1 OR status = 2 OR status = 6 OR assigned_id = '.$this->session->userdata( 'usr_id' ) .'' );
	...
	function get_all_prospects() {
		...
		$this->db->join( 'leadssources', 'leads.source = leadssources.id', 'left' );
		$this->db->join( 'staff', 'leads.assigned_id = staff.id', 'left' );
		$this->db->where( 'change = 1  AND assigned_id = '.$this->session->userdata( 'usr_id' ) .'' );
		$this->db->order_by( 'leads.id', 'desc' );
		return $this->db->get( 'leads' )->result_array();
	}
	function get_all_leads_for_admin() {
		...
		$this->db->join( 'leadssources', 'leads.source = leadssources.id', 'left' );
		$this->db->join( 'staff', 'leads.assigned_id = staff.id', 'left' );
		$this->db->where( 'status = 1 OR status = 2 OR status = 6' );
		$this->db->order_by( 'leads.id', 'desc' );
		return $this->db->get( 'leads' )->result_array();
	}
	function get_all_prospects_for_admin() {
		$this->db->select( '*,leadsstatus.name as statusname,countries.shortname as leadcountry,staff.staffname as leadassigned,staff.staffavatar as assignedavatar,leadssources.name as sourcename,leads.name as leadname,leads.email as leadmail,leads.city as leadcity,leads.title as title,leads.phone as leadphone,leads.id as id' );
		$this->db->join( 'leadsstatus', 'leads.status = leadsstatus.id', 'left' );
		$this->db->join( 'countries', 'leads.country_id = countries.id', 'left' );
		$this->db->join( 'leadssources', 'leads.source = leadssources.id', 'left' );
	...
	function get_prospect_status() {
		return $this->db->get( 'prospectstatus' )->result_array();
	}
	function get_leads_remarks() {
		return $this->db->get( 'remarks' )->result_array();
	}
	...
	function add_remarks( $params ) {
		$this->db->insert( 'remarks', $params );
		return $this->db->insert_id();
	}
	function add_update_status( $params ) {
		$this->db->insert( 'prospect', $params );
		return $this->db->insert_id();
	}
