...
class Api extends CIUIS_Controller {
	...
	function stats() {
		...
		$stats = array(
			...
			'tcl' => $tcl = $this->Report_Model->tcl(),
			'tclc' => $tclc = $this->Report_Model->tclc(),
			'ftcl' => $ftcl = $this->Report_Model->ftcl(),
			'clc' => $clc = $this->Report_Model->clc(),
			'tll' => $tll = $this->Report_Model->tll(),
			'tjl' => $tjl = $this->Report_Model->tjl(),
			...
			'prod' => $prod = $this->Report_Model->prod(),
	...
	function staff_detail( $id ) {
		...
		$properties = array(
			...
			'leads_total' => $this->Staff_Model->total_leads_by_staff( $id ),
	...
	function leftmenu() {
		...
		$all_menu = array(
			...
			'2' => array(
				'title' => lang( 'x_menu_organization' ),
			...
			'4' => array(
				'title' => lang( 'x_menu_prospect' ),
				'show_staff' => 0,
				'url' => base_url( 'prospect' ),
			...
			'7' => array(
				'title' => lang( 'x_menu_products' ),
				'show_staff' => 0,
				'url' => base_url( 'products' ),
				'icon' => 'ico-ciuis-products',
				'path' => 'products',
				'show' => 'false'
			),
			'8' => array(
			...
			'10' => array(
				'title' => lang( 'x_menu_reports' ),
				'show_staff' => 0,
				'url' => base_url( 'report' ),
				'icon' => 'ico-ciuis-reports',
				'path' => 'report',
				'show' => 'false'
			),
	...
	function staff() {
		...
		//$products = $this->Staff_Model->get_services();
		$data_staffs = array();
	...
	function org() {
		$orgs = $this->Staff_Model->get_all_organization();
		//$products = $this->Staff_Model->get_services();
		$data_orgs = array();
		foreach ( $orgs as $org ) {
			$data_orgs[] = array(
	...
	function dueinvoices() {
		$services = $this->Settings_Model->get_plans();
		$data_services = array();
		...
		function services() {
		$services = $this->Settings_Model->get_services();
		$data_services = array();
		foreach ( $services as $service ) {
			$data_services[] = array(
				'id' => $service[ 'id' ],
	...
	function lead( $id ) {
		...
		function remarks( $id ) {
		$remarks = $this->Leads_Model->get_remarks( $id );
		$data_lead = array(
			'id' => $remarks[ 'id' ],
			'lead_id' => $remarks[ 'lead_id' ],
			'remark_desc' => $remarks[ 'name' ],
			'date' => $remarks[ 'date' ],
		);
		echo json_encode( $data_lead );
	}
...
function prospects() {
		if ( !if_admin ) {
			$prospects = $this->Leads_Model->get_all_prospects_for_admin();
		} else {
			$prospects = $this->Leads_Model->get_all_prospects();
		};
		$data_leads = array();
		foreach ( $prospects as $prospect ) {
			$data_leads[] = array(
				...
				'phone' => $prospect[ 'leadphone' ],
				'email' => $prospect[ 'leadmail' ],
				'title' => $prospect[ 'title' ],
				'color' => $prospect[ 'color' ],
				'status' => $prospect[ 'status' ],
				'statusname' => $prospect[ 'statusname' ],
				'source' => $prospect[ 'source' ],
				'sourcename' => $prospect[ 'sourcename' ],
				'assigned' => $prospect[ 'leadassigned' ],
				'avatar' => $prospect[ 'assignedavatar' ],
				'staff' => $prospect[ 'staff_id' ],
				'createddate' => $prospect[ 'created' ],
				'' . lang( 'filterbystatus' ) . '' => $prospect[ 'statusname' ],
				'' . lang( 'filterbysource' ) . '' => $prospect[ 'sourcename' ],
			);
		};
	...
	function prospect( $id ) {
		$prospect = $this->Leads_Model->get_all_prospects( $id );
		
		switch ( $prospect[ 'public' ] ) {
			case '0':
				$is_public = false;
				break;
			case '1':
		...
		$data_lead = array(
			...
			'phone' => $prospect[ 'leadphone' ],
			'assigned' => $prospect[ 'leadassigned' ],
			'assigned_id' => $prospect[ 'assigned_id' ],
			'created' => $prospect[ 'created' ],
			'status_id' => $prospect[ 'status' ],
			'status' => $prospect[ 'statusname' ],
			'source_id' => $prospect[ 'source' ],
			'source' => $prospect[ 'sourcename' ],
			'lastcontact' => $prospect[ 'lastcontact' ],
			'dateassigned' => $prospect[ 'dateassigned' ],
			'staff_id' => $prospect[ 'staff_id' ],
			'dateconverted' => $prospect[ 'dateconverted' ],
			'lost' => $prospect[ 'lost' ],
			'junk' => $prospect[ 'junk' ],
			'public' => $is_public,
			'type' => $is_individual,
	...
	function prospectstatuses() {
		$prospectstatuses = $this->Leads_Model->get_prospect_status();
		echo json_encode( $prospectstatuses );
	}
	function leadremarks() {
		$leadremarks = $this->Leads_Model->get_leads_remarks();
		echo json_encode( $leadremarks );
	}
