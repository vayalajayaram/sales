...
class Login extends CI_Controller {
	...
	function auth() {
		...
		if ( $email && $password && $this->Login_Model->validate_user( $email, $password ) ) {
			...
			$this->session->set_flashdata( 'ntf4', lang( 'invalidpassword' ) );
			$this->show_login( true );
				...
				function authreg() {
        $this->load->model( 'Login_Model' );
		$this->form_validation->set_rules('staffname', 'Your Name','trim|required');
		$this->form_validation->set_rules('email', 'Email ID', 'trim|required');
		$this->form_validation->set_rules('phone', 'Your Mobile ', 'required'); 
		$this->form_validation->set_rules('company', 'Your Company ', 'required');       
		...
		redirect( site_url() . 'login/register' );
			...
			if ($this->Login_Model->insertUser($data)) {
				$this->session->set_flashdata( 'ntf1', lang( 'sucess' ) ); 
		        $this->show_login( true );
				...
				if ( $this->email->send() ) {
					$message = sprintf( lang( 'addedstaff' ), $this->input->post( 'staffname' ) );
					echo $message;
				} else {
					$message = sprintf( lang( 'addedstaffbut' ), $this->input->post( 'staffname' ) );
					echo $message;
				}
