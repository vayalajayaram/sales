			...
			<li ng-repeat="customer in customers | filter: FilteredData | filter:search | pagination : currentPage*itemsPerPage | limitTo: 5" class="ciuis-custom-list-item ciuis-special-list-item lead-name">
				<ul class="list-item-for-custom-list">
					...
					<div class="assigned-staff-for-this-lead user-avatar"><i class="ico-ciuis-staffdetail" style="font-size: 32px"></i></div>
						...
						<a href="<?php echo base_url('customers/customer/')?>{{customer.id}}"><strong ng-bind="customer.clientname"></strong></a><br>
						<small ng-bind="customer.name"></small></a>
						</div>
						<div class="col-md-10">
							<div class="col-md-5">
							<span class="date-start-task"><a href="mailto:{{customer.email}}"><small class="text-muted text-uppercase" ng-bind="customer.email"></small></a><br>
							<strong ng-bind="customer.phone"></strong></span>
							</div>
							<div class="col-md-4">
							<span class="date-start-task"><small class="text-muted text-uppercase" ng-bind="customer.services"></small><br>
							<strong ng-bind="customer.created"></strong></span>
							</div>
  ...
  <md-content layout-padding="">
	<md-content layout-padding>
		...
		<md-input-container  class="md-block">
			<label><?php echo lang('clientname'); ?></label>
			<md-icon md-svg-src="<?php echo base_url('assets/img/icons/individual.svg') ?>"></md-icon>
			<input name="clientname" ng-model="customer.clientname">
		</md-input-container>
		<!--<md-input-container class="md-block">
		...
		<md-input-container class="md-block">
			...
			<label><?php echo lang('productservice'); ?></label>
			<md-select required ng-model="customer.services" name="services" style="min-width: 200px;">
				<md-option ng-value="service.productname" ng-repeat="service in services">{{service.productname}}</md-option>
			</md-select><br>
		</md-input-container>
		<md-input-container class="md-block">
			<label><?php echo lang('phone'); ?></label>
			<!--<input name="phone" ng-model="customer.phone">-->
			<input ng-model="customer.phone" class="form-control" id="title"
    ...
    maxlength = "10"/>
		...
		<md-input-container class="md-block">
			<label><?php echo lang('amountvalue'); ?></label>
			<input type="number" name="taxnumber" ng-model="customer.taxnumber">
		</md-input-container>
		<!--<md-input-container class="md-block">
...
<md-sidenav class="md-sidenav-right md-whiteframe-4dp" md-component-id="ContentFilter">
	 ...
	 <!-- <md-content layout-padding="">
	    ...
	    <md-content layout-padding="">
		<div ng-repeat="(prop, ignoredValue) in customers[0]" ng-init="filter[prop]={}" ng-if="prop != 'id' && prop != 'address' && prop != 'email' && prop != 'phone' && prop != 'balance' && prop != 'customer_id' && prop != 'contacts' && prop != 'services' && prop != 'created' && prop != 'filterbycountry' && prop != 'filterbytype'">
		  <div class="filter col-md-12">
			<h4 class="text-muted text-uppercase"><strong>{{prop}}</strong></h4>
			<hr>
			<div class="labelContainer" ng-repeat="opt in getOptionsFor(prop)">
