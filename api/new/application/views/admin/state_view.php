...
<?php } ?>
						...
						<form  method="post" enctype="multipart/form-data">
						 ...
						 <input type="text" class="form-control1" placeholder="State Name" name="name"  required maxlength="100" />   <input type="number" min="1" class="form-control1" placeholder="Sort Order" name="sort_order"  required  />   <select class="form-control1" name="status" required>  
										 ...
										 &nbsp; 
                          <button  class="btn bg-cyan waves-effect" type="submit">Add State</button>
						  <input type="hidden" name="state_func" value="add">
                        ...
                        <div class="card">
                            <div class="body"><?php if (count($states) > 0 && $states != false) { ?>
                                <table class="table table-bordered table-striped table-hover basic-example" id="bootstrap-table">
									...
									<?php 
									  ...
									  ?>
                                    <tr>
									    <td data-field="name"><?php echo $state['state_name']; ?></td>
                                        
                                        
										
										<td data-field="name"><?php echo $state['sort_order']; ?></td>
										                                        
                                        <td data-field="status"><?php if($state['status']==1){ echo "Enable"; } else { echo "Disabled"; } ?></td>
                                        <td>
                                          <a href="<?php echo base_url(); ?>admin/state/editstate/<?php echo $state['s_id']; ?>"><button class="btn bg-cyan waves-effect" type="button">Edit</button></a>
										<a href="<?php echo base_url(); ?>admin/state/deletestate/<?php echo $state['s_id']; ?>" onclick="return confirm('Are you sure you want to delete this item?');">  <button class="btn bg-red waves-effect" type="button">Delete</button></a> 
                                        </td>
                                    </tr>
									<?php  }  ?>
