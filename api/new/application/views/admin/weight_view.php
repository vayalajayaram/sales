...
<?php } ?>
						...
						<form  method="post" enctype="multipart/form-data">
						 ...
						 <input type="text" class="form-control1" placeholder="Weight in kgs" name="name"  required maxlength="100" />   <input type="number" min="1" class="form-control1" placeholder="Sort Order" name="sort_order"  required  />   <select class="form-control1" name="status" required>  
										 ...
										 &nbsp; 
                          <button  class="btn bg-cyan waves-effect" type="submit">Add Weight</button>
						  <input type="hidden" name="weight_func" value="add">
                        ...
                        <div class="card">
                            <div class="body">
							<?php if (count($weights) > 0 && $weights != false) { ?>
                                <table class="table table-bordered table-striped table-hover basic-example" id="bootstrap-table">
									...
									<?php 
									  ...
									  ?>
                                    <tr>
									    <td data-field="name"><?php echo $weight['weight']; ?></td>
                                        
                                        
										
										<td data-field="name"><?php echo $weight['sort_order']; ?></td>
										                                        
                                        <td data-field="status"><?php if($weight['status']==1){ echo "Enable"; } else { echo "Disabled"; } ?></td>
                                        <td>
                                          <a href="<?php echo base_url(); ?>admin/weight/editweight/<?php echo $weight['w_id']; ?>"><button class="btn bg-cyan waves-effect" type="button">Edit</button></a>
										<a href="<?php echo base_url(); ?>admin/weight/deleteweight/<?php echo $weight['w_id']; ?>" onclick="return confirm('Are you sure you want to delete this item?');">  <button class="btn bg-red waves-effect" type="button">Delete</button></a> 
                                        </td>
                                    </tr>
									<?php  }  ?>
