...
<?php } ?>
						...
						<form  method="post" enctype="multipart/form-data">
						 ...
						 <input type="text" class="form-control1" placeholder="Slot From" name="name[]" id="time" required /> - <input type="text" class="form-control1" placeholder="Slot To" name="name[]" id="time1" required />   <input type="number" min="1" class="form-control1" placeholder="Sort Order" name="sort_order"  required  />   <select class="form-control1" name="status" required>  
										 ...
										 &nbsp; 
                          <button  class="btn bg-cyan waves-effect" type="submit">Add Slot</button>
						  <input type="hidden" name="slot_func" value="add">
                            ...
                            <div class="body"><?php if (count($cities) > 0 && $cities != false) { ?>
                                <table class="table table-bordered table-striped table-hover basic-example" id="bootstrap-table">
									...
									<?php 
									  ...
									  ?>
                                    <tr>
									    <td data-field="name"><?php echo $city['slot_name']; ?></td>
										
										<td data-field="name"><?php echo $city['sort_order']; ?></td>
										                                        
                                        <td data-field="status"><?php if($city['status']==1){ echo "Enable"; } else { echo "Disabled"; } ?></td>
                                        <td>
                                          <a href="<?php echo base_url(); ?>admin/slots/editslot/<?php echo $city['s_id']; ?>"><button class="btn bg-cyan waves-effect" type="button">Edit</button></a>
										<a href="<?php echo base_url(); ?>admin/slots/deleteslot/<?php echo $city['s_id']; ?>" onclick="return confirm('Are you sure you want to delete this item?');">  <button class="btn bg-red waves-effect" type="button">Delete</button></a> 
                                        </td>
                                    </tr>
									<?php  }  ?>
