...
<?php } ?>
                        <div class="card" style="background:none;box-shadow:none;margin-bottom:5px; text-align:right; min-height:20px;">
                           <a href="<?php echo base_url(); ?>admin/coupons/addcoupon"><button class="btn bg-purple" type="button">Add Coupon</button></a>
                        </div>
						
                        <div class="card">
                            <div class="body"><?php if (count($coupons) > 0 && $coupons != false) { ?>
                                <table class="table table-bordered table-striped table-hover basic-example" id="bootstrap-table">
									...
									<?php 
									  ...
									  ?>
                                    <tr>
									     <td data-field="name"><?php echo $coupon['coupon_code']; ?></td>
                                        <td data-field="cname"><?php if($coupon['coupon_type']==1){ echo "Amount"; } else { echo "%"; } ?></td>
                                        <td data-field="sname"><?php echo $coupon['coupon_value']; ?></td>
										<td data-field="stock"><?php echo $coupon['coupon_expiry_date']; ?></td>
                                        
                                        
                                        <td data-field="status"><?php if($coupon['coupon_status']==1){ echo "Enable"; } else { echo "Disabled"; } ?></td>
                                        <td>
                                          <a href="<?php echo base_url(); ?>admin/coupons/editcoupon/<?php echo $coupon['coupon_id']; ?>"><button class="btn bg-cyan waves-effect" type="button">Edit</button></a>
										<a href="<?php echo base_url(); ?>admin/coupons/deletecoupon/<?php echo $coupon['coupon_id']; ?>" onclick="return confirm('Are you sure you want to delete this item?');">  <button class="btn bg-red waves-effect" type="button">Delete</button></a> 
                                        </td>
                                    </tr>
									<?php  }  ?>
