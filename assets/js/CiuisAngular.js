...
function Ciuis_Controller($scope, $http, $mdSidenav, $filter, $interval) {
	...
	$http.get(BASE_URL + 'api/plans').then(function (plans) {
		$scope.plan = Plans.data;
	});
	
	$http.get(BASE_URL + 'api/services').then(function (Services) {
...
function Leads_Controller($scope, $http, $mdSidenav, $mdDialog) {
	...
	$scope.Updatee = buildToggler('Updatee');
	...
	$http.get(BASE_URL + 'api/prospects').then(function (Prospects) {
		$scope.prospects = Prospects.data;
	});
	...
	$http.get(BASE_URL + 'api/leads_by_leadsource_leadpage').then(function (LeadsBySource) {
		...
		$http.get(BASE_URL + 'api/org').then(function (Orgs) {
		$scope.orgs = Orgs.data;
		
			$scope.Addorg = function () {
			var dataObj = $.param({
				company: $scope.org.org_name,
				status: $scope.lead.inactive,
				city: $scope.lead.source_id,
	...
	$http.get(BASE_URL + 'api/leadremarks').then(function (LeadRemarks) {
		$scope.leadremarks = LeadRemarks.data;
		$scope.remarks = function (leadid) {
			// Appending dialog to document.body to cover sidenav in docs app
			var confirm = $mdDialog.prompt()
				.title('Remarks')
				.textContent('Please type Remarks')
				.placeholder('Remarks')
				.ariaLabel('Remarks')
				.initialValue('')
				.required(true)
			...
			$mdDialog.show(confirm).then(function (result) {
				...
				$http.post(BASE_URL + 'leads/add_remarks/', dataObj, config)
					.then(
						function (response) {
							console.log(response);
		...
		//$http.get(BASE_URL + 'api/leadremarks').then(function (LeadStatus) {
		//$scope.leadStatus = LeadStatus.data;
		$scope.leadStatus = function (leadid) {
			// Appending dialog to document.body to cover sidenav in docs app
			var confirm = $mdDialog.prompt()
				.title('Update Status')
				.textContent('Please type Status')
				.placeholder('Appointment or Follow Up')
				.ariaLabel('Status')
				.initialValue('')
				.required(true)
				.ok('Update!')
			...
			$mdDialog.show(confirm).then(function (result) {
				...
				$http.post(BASE_URL + 'leads/addupdate_status/', dataObj, config)
					.then(
						function (response) {
							console.log(response);
							$scope.leadstatuses.push({
...
function Lead_Controller($scope, $http, $mdSidenav, $mdDialog, $filter) {
	...
	$scope.ReminderForms = buildToggler('ReminderForms');
	$scope.Update = buildToggler('Update');
	$scope.StatusUpdate = buildToggler('StatusUpdate');
	...
	$scope.close = function () {
		...
		$mdSidenav('ReminderForms').close();
		$mdSidenav('Update').close();
		$mdSidenav('StatusUpdate').close();
	};
	...
	$http.get(BASE_URL + 'api/lead/' + LEADID).then(function (Lead) {
		...
		$scope.UpdateLead = function () {
			...
			var posturl = BASE_URL + 'leads/Statusupdate/' + LEADID;
			$http.post(posturl, dataObj, config)
				.then(
					function (response) {
						console.log(response);
	...
	$scope.AppointmentConvert = function () {
		// Appending dialog to document.body to cover sidenav in docs app
		var confirm = $mdDialog.confirm()
			.title('Convert FollowUp to Appointment')
			.textContent('Do you want to convert lead into a Appintment?')
			.ariaLabel('Convert Lead')
			.targetEvent(LEADID)
			.ok('Convert!')
			.cancel('Cancel');
		...
		$mdDialog.show(confirm).then(function () {
			...
			$http.post(BASE_URL + 'leads/appconvert/' + LEADID, config)
				.then(
			
						function (response) {
						console.log(response);
						window.location.href = BASE_URL + 'Prospect/lead/' + LEADID;
					},
				);
		...
		// Appending dialog to document.body to cover sidenav in docs app
		var confirm = $mdDialog.confirm()
			.title('Convert Prospect to Lead')
			.textContent('Do to Not Interested?')
			.ariaLabel('Convert Lead')
			.targetEvent(LEADID)
			.ok('Convert!')
			.cancel('Cancel');
		...
		$mdDialog.show(confirm).then(function () {
			...
			$http.post(BASE_URL + 'leads/notinterest/' + LEADID, config)
				.then(
	...
	$http.get(BASE_URL + 'api/reminders_by_type/lead/' + LEADID).then(function (Reminders) {
		...
		$scope.AddFollowReminder = function () {
			var dataObj = $.param({
				description: $scope.reminder_description,
				date: moment($scope.reminder_date).format("YYYY-MM-DD HH:mm:ss"),
				staff: $scope.reminder_staff,
			...
			var posturl = BASE_URL + 'trivia/addfollowreminder';
			$http.post(posturl, dataObj, config)
				.then(
					function (response) {
						console.log(response);
					...
					function (response) {
						console.log(response);
						$http.post(BASE_URL + 'prospects/lead/' + LEADID, dataObj, config)
					}
				);
		};
	...
	$http.get(BASE_URL + 'api/customers').then(function (Customers) {
		...
		$scope.AddCustomer = function () {
			var dataObj = $.param({
				...
				clientname: $scope.customer.clientname,
				...
				services: $scope.customer.services,
...
function Staffs_Controller($scope, $http, $mdSidenav, $mdDialog) {
	...
	$http.get(BASE_URL + 'api/staff/').then(function (Staff) {
		...
		$http.get(BASE_URL + 'api/org/').then(function (Org) {
		$scope.org = Org.data;
			$scope.Addorg = function () {
			var dataObj = $.param({
				org_name: $scope.org.org_name,
				email: $scope.org.email,
				country: $scope.org.country,
				state: $scope.org.state,
				city: $scope.org.city,
				mobile: $scope.org.mobile,
				telephone: $scope.org.telephone,
				website_url: $scope.org.website_url,
				plan_id: $scope.org.plan_id,
				inactive: $scope.org.active,
				password: $scope.passwordNew,
			});
			var config = {
				headers: {
			...
			var posturl = BASE_URL + 'Organizations/create/';
			$http.post(posturl, dataObj, config)
				.then(
					function (response) {
						console.log(response);
...
function Staff_Controller($scope, $http, $mdSidenav, $mdDialog, $filter) {
	...
	$scope.Update = buildToggler('Update');
	$scope.Updateprofile = buildToggler('Updateprofile');
